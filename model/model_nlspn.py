...
from .nlspn.common_nlspn import *
from .nlspn.modulated_deform_conv_func import ModulatedDeformConvFunction

class NLSPN(nn.Module):
    def __init__(self, args, ch_g, ch_f, k_g, k_f,
    prop_time=18, affinity='TGASS', affinity_gamma=0.5,
    conf_prop=True, legacy=False, preserve_input=False):
        ...
        if self.affinity in ['AS', 'ASS', 'TC', 'TGASS']:
            ...
            if self.affinity == 'TC':
                ...
                self.aff_scale_const = nn.Parameter(
                    affinity_gamma * self.num * torch.ones(1))
            else:
...
class BaseNLSPNModel(nn.Module):
    def __init__(self, args, conf_prop=True, network='resnet34',
                 prop_kernel=3, from_scratch=False, nlspn=True):
        super(BaseNLSPNModel, self).__init__()
    ...
    def forward(self, sample):
        ...
        out = torch.clamp(pred_init, min=0)

        return out, fe1, fe2, fd2, pred_init, id_fd1

class NLSPNModel(BaseNLSPNModel):
    def forward(self, sample):
        out, fe1, fe2, fd2, pred_init, id_fd1 = super().forward(sample)
        ...
        if self.nlspn:
            ...
            y, y_inter, offset, aff, aff_const = \
                self.prop_layer(pred_init, guide, confidence, sample['d'], sample['rgb'])

            # Remove negative depth
            out = torch.clamp(y, min=0)

        return out

class NLSPNModelConf(BaseNLSPNModel):
    def __init__(self, args, conf_prop=True, network='resnet34',
                 prop_kernel=3, from_scratch=False, nlspn=True):
        super(NLSPNModelConf, self).__init__(args, conf_prop, network, prop_kernel, from_scratch, nlspn)
        self.id_conf1 = conv_bn_relu(64+64, 64, kernel=3, stride=1,
                                    padding=1)
        self.id_conf = convx2_bn_relu(64+64, 64, 1, kernel=3, stride=1,
                                    padding=1)
    ...
    def forward(self, sample):
        ...
        if self.nlspn:
            ...
            pred_conf_inv = self.id_conf(self._concat(id_fd1, fe1))
