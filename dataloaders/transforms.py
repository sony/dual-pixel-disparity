...
class Compose(object):
    ...
    def __call__(self, img):
        for i, t in enumerate(self.transforms):
            # print("Compose img.shape b", i, img.shape, img.dtype)
            img = t(img)
            # print("Compose img.shape a", i, img.shape, img.dtype)
        return img


class ToTensor(object):
    """Convert a ``numpy.ndarray`` or a ``float`` to tensor.

    Converts a numpy.ndarray (H x W x C) to a torch.FloatTensor of shape (C x H x W).
    Converts a float to a torch.FloatTensor of shape (1,).
    """
    def __call__(self, data):
        """Convert a ``numpy.ndarray`` or a ``float`` to tensor.

        Args:
            data (numpy.ndarray, float): Data to be converted to tensor.

        Returns:
            Tensor: Converted data.
        """
        if isinstance(data, np.ndarray):
            if not _is_numpy_image(data):
                raise TypeError('img should be ndarray. Got {}'.format(type(data)))

            if data.ndim == 3:
                data = torch.from_numpy(data.transpose((2, 0, 1)).copy())
            elif data.ndim == 2:
            ...
            raise TypeError('data should be float or ndarray. Got {}'.format(type(data)))
...
class Resize(object):
    ...
    def __call__(self, img):
        ...
        """
        if img.ndim == 3:
            out = cv2.resize(img, dsize=self.size, interpolation=self.interpolation)
            if out.ndim == 2:
